...
function initializeAdSDKs() {
  ...
  const checkGigaPub = setInterval(() => {
    if (window.gigapub && typeof window.gigapub.show === 'function') {
      ...
      // Setup GigaPub configuration
      window.gigapub.config({
        onAdComplete: () => {
...
async function playGigaPubAd() {
  return new Promise((resolve, reject) => {
    ...
    try {
      // Method 1: Using gigapub.show()
      if (window.gigapub && typeof window.gigapub.show === 'function') {
        window.gigapub.show({
          onComplete: () => {
            console.log('GigaPub ad completed via show()');
            resolve();
          },
          onError: (error) => {
            console.error('GigaPub ad error:', error);
          ...
          onSkip: () => {
            console.log('GigaPub ad skipped');
      ...
      // Method 2: Using global showGiga function
      ...
      // Method 3: Fallback - assume ad plays for 3 seconds
      console.warn('GigaPub SDK found but show method unavailable, using fallback');
      setTimeout(resolve, 3000);

    } catch (error) {
      console.error('GigaPub playback error:', error);
      reject(error);
    }
  });
}
