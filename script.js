// Firebase setup (NO trailing spaces!)
...
// ==================== SDK WAIT UTIL ====================
/**
 * Wait for a global ad function (e.g., showGiga) to become available
 */
function waitForFunction(fnName, timeout = 8000, interval = 250) {
  return new Promise((resolve, reject) => {
    ...
    const check = () => {
      if (typeof window[fnName] === 'function') {
        ...
        reject(new Error(`${fnName} not loaded after ${timeout}ms`));
      } else {
...
// ==================== PRIZE RULES ====================
const PRIZE_RULES = {
  1: 15, 2: 10, 3: 7, 4: 6, 5: 5,
  6: 4, 7: 3.5, 8: 3, 9: 2.5, 10: 2
};
const RANGE_RULES = [
...
function setupRealtimeListeners() {
  onValue(ref(db, 'Target'), (s) => { if (s.exists()) { target = s.val(); document.getElementById('target').textContent = target; updateDisplay(); } });
  onValue(ref(db, 'session'), (s) => { if (s.exists()) { session = s.val(); document.getElementById('sessionDisplay').textContent = `(Session: ${session})`; } });
  onValue(ref(db, 'price_pool'), (s) => { if (s.exists()) { prizePool = s.val(); document.getElementById('prizePoolDisplay').textContent = prizePool; } });
  onValue(ref(db, 'users'), (s) => {
    if (s.exists()) {
      const users = s.val();
      leaderboard = Object.entries(users).map(([uid, info]) => ({ userId: uid, ads: info.AdsWatched || 0 }));
      globalWatched = leaderboard.reduce((sum, u) => sum + u.ads, 0);
      updateDisplay();
      updateYouPage();
...
function updateDisplay() {
  ...
  document.getElementById('progress').style.width = `${Math.min((globalWatched / target) * 100, 100)}%`;
}
...
async function playAd(adType) {
  ...
  try {
    if (adType === 'LibTL') {
      const showFn = await waitForFunction('show_10142875');
      promise = showFn();
    } else if (adType === 'Giga') {
      const showFn = await waitForFunction('showGiga');
      promise = showFn();
    } else {
    ...
    showToast(`❌ ${adType} ad failed or skipped`);
  } finally {
    resetUI();
  }
}
